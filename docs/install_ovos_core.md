...
**NOTE** The GUI requires seperate packages in addition to what is required by ovos-core.  The [GUI installation](install_gui.md) is covered in its own section.
...
A few packages are not necessasary, but are needed to install from source and may be required for some plugins.  To add these packages run this command.
...
**NOTE** This will install `lgpl` licenced software.
...
### Individual Modules
Each module can be installed independently to only inclue the parts needed or wanted for a specific system.
...
## From Source
We will use git to clone the repositories to a local directory.  While not specificly necessasary, we are assuming this to be the users `HOME` directory.
...
Some systems may not require a full install of OVOS.  Luckly, it can be installed as individual modules.
...
This is the nervous system of OVOS needed for modules to talk to eachother

[Starting the Messagebus](starting_modules.md#messagebus)

**listener**

OVOS has updated thier listener to use `ovos-dinkum-listener` instead of `ovos-listener`.  It is code from [mycroft-dinkum](https://github.com/MycroftAI/mycroft-dinkum) adopted for use with the OVOS ecosystem.  Previous listeners are still available, but not recommended.
...
The OVOS Plugin based Hardware Abstraction Layer is what is used to allow the OVOS software to communicate with hardware devices such as the [OS system](https://github.com/OpenVoiceOS/ovos-PHAL-plugin-system) or interacting with the [Mycroft Mark1 device](https://github.com/OpenVoiceOS/ovos-PHAL-plugin-mk1)

The PHAL system consists of two interfaces.

`ovos-phal` is the basic interface that normal plugins would use.

`ovos-admin-phal` is used where superuser privleges are needed.

**Be extreamly careful when installing `admin-phal` plugins as it gives full control over the host system**
...
It also installs [OVOS Common Play](https://github.com/OpenVoiceOS/ovos-ocp-audio-plugin) which can be used as a stand alone media player and is required for OVOS audio playback.

`git clone https://github.com/OpenVoiceOS/ovos-audio`

`pip install ~/ovos-audio`

This will enable a default TTS engine for voice feedback from your OVOS device.  There are plenty of options available here also.

[Starting Audio](starting_modules.md#audio)

You now should have all of the seperate components needed to run a full OVOS software stack.
